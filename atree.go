...
func intervalOverlaps(n *node, low, high Value, interval Interval, maxDimension uint64) bool {
...
func overlaps(high, otherHigh, low, otherLow Value) bool {
	return high.Greater(otherLow) && low.Lesser(otherHigh)
}

// compare returns an int indicating which direction the node
// should go.
func compare(nodeLow, ivLow Value, nodeID, ivID uint64) int {
	if ivLow.Greater(nodeLow) {
		return 1
	}

	if ivLow.Lesser(nodeLow) {
		return 0
	}
...
func newNode(interval Interval, min, max Value, dimension uint64) *node {
	itn := &node{
		interval: interval,
		min:      min,
		max:      max,
...
func insertInterval(dimension uint64, interval Interval, index Value, count int64) int {
	low, high := interval.LowAtDimension(dimension), interval.HighAtDimension(dimension)
	if index.GreaterOrEq(high) {
		return 0
	}
...
index Value, count int64) (Intervals, Intervals) {
	...
	tree.root.query(index.MinimalValue(), index.MaximalValue(), nil, tree.maxDimension, func(n *node) {
		if dimension > 1 {
		...
		if n.max.GreaterOrEq(index) {
			// won't change min or max in this case
			return
		}

		n.max.Add(count)
		if n.min.GreaterOrEq(index) {
			n.min.Add(count)
		}

		mod := false
		if n.high.Greater(index) {
			n.high.Add(count)
			if n.high.Lesser(index) {
				n.high = index
			}
			mod = true
		}
...
func setMax(parent *node) {
	...
	if parent.children[0] != nil && parent.children[0].max.Greater(parent.max) {
		parent.max = parent.children[0].max
	}

	if parent.children[1] != nil && parent.children[1].max.Greater(parent.max) {
...
func setMin(parent *node) {
	...
	if parent.children[0] != nil && parent.children[0].min.Lesser(parent.min) {
		parent.min = parent.children[0].min
	}

	if parent.children[1] != nil && parent.children[1].min.Lesser(parent.min) {
		parent.min = parent.children[1].min
	}

	if parent.low.Lesser(parent.min) {
		parent.min = parent.low
	}
}
